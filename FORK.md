...
| 能力 | 关键文件 | 不变量 |
| --- | --- | --- |
| Pi Package 支持 | `package.json` | `pi.extensions` 加载三个本地 extension，`pi.skills` 保持为空 |
| Pi 精选与全量配置 | `pi-profiles.json`、`extensions/pi-skill-profiles.ts` | 默认精选 13 个；全量自动覆盖 Engineering 和 Productivity 的稳定 skills |
| Skill 斜杠命令兼容 | `extensions/pi_slash_compatible.ts` | 当前 profile 内的 `/<skill-name>` 转发到 `/skill:<skill-name>` |
| Claude Code `/clear` 兼容 | `extensions/claude-clear.ts` | Pi 中的 `/clear` 开始新会话 |
